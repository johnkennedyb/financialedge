...
import { readImportedPage } from "@/lib/local-content";
...
function cleanWordPressContent(html: string): string {
    ...
    let cleaned = html
        ...
        .replace(/\[\/vc_column_inner\]/g, '')
        .replace(/\[td_block_text_with_title[^\]]*\]/g, '<h3>')
        .replace(/\[\/td_block_text_with_title\]/g, '</h3>')
        .replace(/\[td_block_2[^\]]*\]/g, '')
        .replace(/\[td_block_ad_box[^\]]*\]/g, '')
        .replace(/\[vc_raw_html[^\]]*\]JTNDaDQlMjBjbGFzcyUzRCUyMmJsb2NrLXRpdGxlJTIyJTNFJTNDc3BhbiUzRVNlbmQlMjB1cyUyMGElMjBtZXNzYWdlJTIxJTNDJTJGc3BhbiUzRSUzQyUyRmg0JTNF\[\/vc_raw_html\]/g, '<h4>Send us a message!</h4>')
        .replace(/custom_title="([^"]+)"/g, '$1')
        .replace(/limit="[^"]+"/g, '')
        .replace(/spot_id="[^"]+"/g, '')
        .replace(/spot_title="[^"]+"/g, '');
...
export default async function AboutPage() {
    ...
    return (
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
            ...
            <section className="card animate-fe-fade-up overflow-hidden rounded-[28px] p-6 md:p-8">
            ...
            <div className="card animate-fe-fade-up overflow-hidden rounded-[28px] p-6 md:p-8">
                <div
