    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 btn-modern">Retry</button>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {debugInfo && <p className="text-xs text-accent mt-1">Debug: {debugInfo}</p>}
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <input
              ...
              className="rounded-lg border border-border bg-background px-4 py-2 pl-10 text-sm focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent w-48 md:w-64"
            />
          ...
          <button onClick={() => setLastRefresh(new Date())} disabled={loading} className="rounded-lg border border-border bg-background px-3 py-2 text-sm hover:bg-secondary disabled:opacity-50">
            {loading ? "Loading..." : "🔄 Refresh"}
          </button>
          <button onClick={() => setView(view === "grid" ? "list" : "grid")} className="rounded-lg border border-border bg-background px-3 py-2 text-sm hover:bg-secondary">
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          />
          <button onClick={handleUpload} disabled={!selectedFile || uploading} className="btn-modern disabled:opacity-50">
            {uploading ? "Uploading..." : "Upload"}
          </button>
        </div>
        {selectedFile && <div className="mt-4 text-sm text-muted">Selected: {selectedFile.name} ({formatFileSize(selectedFile.size)})</div>}
      </div>
              ...
              <div className="aspect-video bg-secondary/20 relative">
                {file.type === "image" ? (
                  ...
                  <div className="flex items-center justify-center h-full">
                    <span className="text-3xl">{file.type === "video" ? "🎥" : "📄"}</span>
                  </div>
                )}
              </div>
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mt-3">
                  <button className="text-accent hover:text-accent/80 text-sm font-medium">Edit</button>
                  <button onClick={() => handleDelete(file.id)} className="text-destructive hover:text-destructive/80 text-sm font-medium">Delete</button>
                </div>
              </div>
            </div>
          ))}
                  ...
                  <tr key={file.id} className="hover:bg-secondary/30">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <span className="text-lg">{file.type === "image" ? "🖼️" : file.type === "video" ? "🎥" : "📄"}</span>
                        <div>
                          <p className="font-medium">{file.name}</p>
                          {file.dimensions && <p className="text-sm text-muted">{file.dimensions.width} × {file.dimensions.height}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 rounded-full text-xs bg-accent/10 text-accent capitalize">{file.type}</span>
                    </td>
      ...
      {searchQuery && filteredFiles.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted">No media files found matching &quot;{searchQuery}&quot;.</p>
          <button onClick={() => setSearchQuery("")} className="mt-2 text-accent hover:underline text-sm">Clear search</button>
        </div>
      )}
