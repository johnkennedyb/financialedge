...
(()=>{
    const e = new Error("Cannot find module './local-data'");
...
async function getLatestPosts(perPage = 12) {
    const posts = getAllPosts().filter((post)=>post.status === 'publish').sort((a, b)=>new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, perPage);
...
async function getPostsByCategorySlug(slug, perPage = 12) {
    ...
    const posts = getAllPosts().filter((post)=>post.status === 'publish' && post.categories.includes(slug)).sort((a, b)=>new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, perPage);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b05adeb2._.js.map